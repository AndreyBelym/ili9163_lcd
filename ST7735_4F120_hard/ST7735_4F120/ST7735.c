...
#define TFT_SDA									(*((volatile unsigned long *)0x40004080))
#define TFT_SDA_HIGH						0x20
#define TFT_SDA_LOW							0
#define TFT_SCK									(*((volatile unsigned long *)0x40004010))
#define TFT_SCK_HIGH						0x04
#define TFT_SCK_LOW							0
...
void static transmit_byte(unsigned char c){
	int i=8;
	TFT_CS=TFT_CS_LOW;
	while(i){
		TFT_SDA=(c&0x80)?TFT_SDA_HIGH:TFT_SDA_LOW;
		--i;c<<=1;
		SysTick_Wait(CLOCK_DELAY);
		TFT_SCK=TFT_SCK_HIGH;
		SysTick_Wait(CLOCK_DELAY);
		TFT_SCK=TFT_SCK_LOW;
...
void transmit_receive(unsigned char c,unsigned char cnt,unsigned char *r){
	int i=8,n=0;
	DC = DC_COMMAND;
	TFT_CS=TFT_CS_LOW;
	while(i){
	...
	GPIO_PORTA_DIR_R&=~TFT_SDA_HIGH;
	GPIO_PORTA_PUR_R|=TFT_SDA_HIGH;
	//Dummy clock
	if(cnt>1){
		TFT_SCK=TFT_SCK_HIGH;
		SysTick_Wait(CLOCK_DELAY);
...
void GPIO_Init(){
  ...
  SYSCTL_RCGC2_R |= SYSCTL_RCGC2_GPIOA; // activate port A
  delay = SYSCTL_RCGC2_R;               // allow time to finish activating
  GPIO_PORTA_DIR_R |= 0xEC;             // make PA6,7 out
  GPIO_PORTA_AFSEL_R &= ~(0xEC);          // disable alt funct on PA6,7
  GPIO_PORTA_DEN_R |= 0xEC;             // enable digital I/O on PA2,3,5,6,7
                                        // configure PA2,3,5 as SSI
  GPIO_PORTA_PCTL_R &= ~0xFFF0FF00;
...
void HX8353_Init(void)
{	
	int i=0;
	GPIO_Init();

	writecommand(0xB9);//SETEXTC
	writedata(0xFF);
	writedata(0x83);
	writedata(0x53);
	
	writecommand(0xB0);//RADJ
	writedata(0x3C);
	writedata(0x01);
	
	
	writecommand(0xB6);//VCOM
	writedata(0x94);
	writedata(0x6C);
	writedata(0x50);   
	
	writecommand(0xB1);//PWR
	writedata(0x00);
	writedata(0x01);
	writedata(0x1B);
	writedata(0x03);
	...
	writecommand(0xE0); //Gamma setting for tpo Panel
	writedata(0x50);
	writedata(0x77);
	writedata(0x40);
	writedata(0x08);
